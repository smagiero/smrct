...
// iverilog -s pll_avg_tb -g2012 -Wall -Wno-sensitivity-entire-vector -Wno-sensitivity-entire-array -o pll_avg ../tb/pll_avg_tb.v
// ./pll_avg
// surfer pll_avg.vcd

`define VFO_Delta    0.001  // ns.
`define VFO_MaxDelta 0.250  // ns.
//
`define ClockHalfPeriod 1.25   // ns half-period at 400 MHz.
`define VFOBaseDelay `ClockHalfPeriod

`define VFO_Max (`VFOBaseDelay*1.0 + `VFO_MaxDelta*1.0)
`define VFO_Min (`VFOBaseDelay*1.0 - `VFO_MaxDelta*1.0)

`include "../behavioral/pll_avg_behav.v"
...
pll_avg PLL1 (.ClockOut(PLLClockOut), .ClockIn(ClockInStim), .Reset(Reset));
